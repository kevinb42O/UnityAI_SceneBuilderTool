...
public class AAAMovementController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (controller.enabled && !isSliding) {
            ...
            {
                Debug.Log($"[CONTROLLER.MOVE SPEED LOSS] PreMove: {preMoveHorizSpeed:F0} u/s → PostMove: {postMoveHorizSpeed:F0} u/s (Lost {preMoveHorizSpeed - postMoveHorizSpeed:F0} u/s)");
            }
                ...
                if (IsGrounded)
                {
                    ...
                    if (enableAccelerationSystem)
                    {
                        ...
                        if (hasInput)
                        {
                            ...
                            // 🐛 BUG FIX: DISABLE momentum conservation on slopes - it causes massive speed crashes!
                            ...
                            // 🔥 CRITICAL: Only use momentum conservation on FLAT GROUND for slides/air tricks
