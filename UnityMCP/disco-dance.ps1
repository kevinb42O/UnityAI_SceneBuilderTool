# Dancing Animation with DISCO COLORS! 🌈✨
Write-Host "Starting DISCO Dance Animation! 🕺💃🌈" -ForegroundColor Cyan
...
curl -Method POST -Uri "http://localhost:8765/createGameObject" -ContentType "application/json" -Body '{"name": "LeftBoot", "position": {"x": -0.15, "y": -0.75, "z": 0.05}, "rotation": {"x": 0, "y": 0, "z": 0}, "scale": {"x": 0.2, "y": 0.15, "z": 0.3}}' -UseBasicParsing | Out-Null
...
Write-Host ""
Write-Host "Base pose set! Starting DISCO dance in 3..." -ForegroundColor Green
Start-Sleep -Seconds 1
Write-Host "2..." -ForegroundColor Green
Start-Sleep -Seconds 1
Write-Host "1..." -ForegroundColor Green
Start-Sleep -Seconds 1
Write-Host "LET'S DISCO! 🪩✨🌈" -ForegroundColor Magenta
Write-Host ""
...
while ($true) {
    ...
    # COLOR CYCLING! 🌈
    # Change colors every 10 frames
    if ($frame % 5 -eq 0) {
        $colorIndex = ($colorIndex + 1) % $rainbowColors.Count
        $color = $rainbowColors[$colorIndex]
        
        # Cycle through body parts with different colors
        $color2 = $rainbowColors[($colorIndex + 2) % $rainbowColors.Count]
        $color3 = $rainbowColors[($colorIndex + 4) % $rainbowColors.Count]
        
        # Torso gets main color
        $scaleEffect = 0.6 + ($bounce * 0.05)  # Pulse effect
        curl -Method POST -Uri "http://localhost:8765/setTransform" -ContentType "application/json" -Body "{`"name`": `"Torso`", `"scale`": {`"x`": $scaleEffect, `"y`": 0.8, `"z`": 0.35}}" -UseBasicParsing | Out-Null
        
        # Head gets second color with pulse
        $headScale = 0.4 + ($bounce * 0.03)
        curl -Method POST -Uri "http://localhost:8765/setTransform" -ContentType "application/json" -Body "{`"name`": `"Head`", `"scale`": {`"x`": $headScale, `"y`": 0.3, `"z`": $headScale}}" -UseBasicParsing | Out-Null
        
        # Visor gets third color (glowing effect)
        $visorScale = 0.35 + ($bounce * 0.05)
        curl -Method POST -Uri "http://localhost:8765/setTransform" -ContentType "application/json" -Body "{`"name`": `"Visor`", `"scale`": {`"x`": $visorScale, `"y`": 0.15, `"z`": 0.05}}" -UseBasicParsing | Out-Null
    }
    ...
    # VISOR - Follow head with GLOW pulse
    ...
    # Progress indicator with COLOR and dance emoji
    if ($frame % 5 -eq 0) {
        $danceEmojis = @("🕺", "💃", "🎵", "🎶", "✨", "🌟", "🪩", "🌈")
        $emoji = $danceEmojis[$frame % 8]
        $colorName = @("RED", "ORANGE", "YELLOW", "GREEN", "BLUE", "PURPLE", "MAGENTA")[$colorIndex]
        Write-Host "`rDISCO $emoji $colorName Frame: $frame" -NoNewline -ForegroundColor Magenta
    }
